...
<nav class="navbar navbar-fixed-top">
  ...
  <body class="text-center">
    <form class="form-signin" action="mydetails.php" " method="post">
      ...
      <input type="text"  name="nameid" class="form-control" value="<?php echo $name; ?>" required autofocus><br>

        <label style="float: left;font-size: 18px;">Email </label>
      <input type="email"  name="emailid" class="form-control" value="<?php echo $email; ?>"  required autofocus><br>
      
        <label style="float: left;font-size: 18px;">Password</label>
      <input type="text"  name="passwordid" class="form-control"  value="<?php echo $password; ?>"  required><br>
       <label style="float: left;font-size: 18px;">Address</label>
       <input type="text"  name="addressid" class="form-control"  value="<?php echo $address; ?>"  required><br>

       <label style="float: left;font-size: 18px;">Contact</label>
      <input type="text" name="phoneid" class="form-control" value="<?php echo $phone; ?>" required><br>
      ...
      if(isset($_POST['submit']))
      {
        ...
        $phone=$_POST['phoneid'];
        $query="UPDATE register SET name='$name' , email='$email' , password='$password' , address='$address' , phone='$phone' WHERE email='$emailid' " ;
         $result1=mysqli_query($con,$query);

 
           if($result1)
           {
            header('Refresh:2;url:mydetails.php');
           }
           else
           {
